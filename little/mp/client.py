...
class GameClient(object):
    ...
    def send(self, request, args=None):
        ...
        logging.info('Request to be sent: {0}'.format(request))
        # Bounce the server
        self.disconnect()
        logging.info('****** Starting Request   *******')
        self.connect(ip=self.ip)
        logging.info('Connected')
        logging.info('Sending request to server: {0}'.format(str(request)))
        ...
        try:
            ...
            if buffer_size is not None:
                logging.info('--Received buffer size from server: {0}'.format(buffer_size))
            else:
                raise RuntimeError('Did not receive buffer size from server!')
        except socket.error:
            logging.info(socket.error.message)
            self.disconnect()
        ...
        logging.info('--Listening for payload...')
        payload = self.receive_packet(buffer_size, raise_error=True)
        if not payload:
            logging.info('--Payload not found, retrying')
    ...
    def receive_packet(self, buffer_size, raise_error=False):
        logging.info('Expecting packet size: {0}'.format(buffer_size))
        packet = self.server.recv(buffer_size)

        actual_size = sys.getsizeof(packet)
        logging.info('Actual packet size: {0}'.format(actual_size))
        if raise_error:
            if actual_size != buffer_size:
                self.disconnect()
                logging.info('Packet size mismatch has occured: Expected {0}, Got: {1}'.format(buffer_size, actual_size))
                raise PacketSizeMismatch('Expected size: {0}, Actual size: {1}'.format(buffer_size, actual_size))
        if packet:
            data = pickle.loads(packet)
            logging.info('received the following data: {0}'.format(data))
            return data
        else:
            logging.info('Packet empty, returning None')
            return None
