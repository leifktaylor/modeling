    ...
    def login(self, request):
        ...
        if self.server.authenticate_credentials(request):
            if request['charactername'] not in self.goc.playernames:
                logging.info('Player logging in, adding player Lifeform to GOC')
                logging.info('Loading mp/users/{0}/sav'.format(request['charactername']))
                gameobject, id = self.goc.load_gameobject('mp/users/{0}.sav'.format(request['charactername']))
                logging.info('Loaded, creating gameobject')
                logging.info('Created gameobject with id: {0}'.format(id))
    ...
    def logout(self, request):
        ...
        if self.server.authenticate_credentials(request):
            if request['charactername'] in self.goc.playernames:
                logging.info('Removing gameobject with id: {0}'.format(request['id']))
            ...
            logging.info('User had invalid credentials, halting action')
            return {'status': -1, 'response': {'message': 'Credentials invalid'}}
...
class GameServer(object):
    ...
    def logout_client(self, request):
        ...
        if charactername not in self.goc.playernames:
            logging.info('Cannot logout, user is not logged in')
    ...
    def listen(self, clients):
        try:
            ...
            for client in clients_list:
                logging.info('--Client conneted, listening...')
                request = self.receive_packet(client, BUFFER_SIZE)
                if request:
                    if request == -1:
                        logging.info('--Client returned error, disconnecting')
                        client.close()
                    else:
                        try:
                        ...
                        if VERBOSE: logging.info('--Sending buffer size: {0}'.format(buffer_size))
                        client.send(response)

                        # Wait for confirmation from client for buffer size
                        if VERBOSE: logging.info('--awaiting client response to send payload')
                        client_response = self.receive_packet(client, 128)

                        if client_response:
                            if VERBOSE: logging.info('--client response to send payload received:{0}'.format(client_response))
                            if client_response == buffer_size:
                                if VERBOSE: logging.info('--Sending payload (size): {0}'.format(buffer_size))
                                if VERBOSE: logging.info('--Sending payload data: {0}'.format(payload_string))
                                client.send(payload)
                            elif client_response == -1:
                                logging.info('--Received error message from client')

                        # Client needs to confirm that it received payload

                logging.info('Socket closed with client')
                client.close()
